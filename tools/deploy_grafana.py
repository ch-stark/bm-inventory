...
SERVICE_NAME = args.service or 'grafana'
...
def deploy_oauth_reqs():
    ...
    if not utils.check_if_exists('sa', SERVICE_NAME):
        cmd = "{} -n {} create serviceaccount {} ".format(CMD_BIN, NAMESPACE, SERVICE_NAME)
        utils.check_output(cmd)
    ...
    cmd = "{} -n {} annotate serviceaccount {} --overwrite {}='{}'".format(
            CMD_BIN, NAMESPACE, SERVICE_NAME, annotation_name, json_manifest)
    utils.check_output(cmd)
...
def deploy_grafana_route():
    ...
    ingress_domain = utils.get_domain(namespace=NAMESPACE)
    with open(src_file, "r") as src:
        with open(dst_file, "w+") as dst:
            data = src.read()
...
def deploy_grafana_ds():
    ...
    if not utils.check_if_exists('secret', secret_name):
        ...
        cmd = "{} create secret generic {} --namespace={} --from-file=prometheus.yaml={}".format(CMD_BIN, secret_name, NAMESPACE, src_file)
        utils.check_output(cmd)
...
def deploy_grafana_config(conf_file):
    ...
    if not utils.check_if_exists('secret', secret_name):
        ...
        cmd = "{} create secret generic {} --namespace={} --from-file=grafana.ini={}".format(CMD_BIN, secret_name, NAMESPACE, src_file)
        utils.check_output(cmd)
    else:
...
def main():
    if args.target != "oc-ingress":
        ...
        sleep(10)
        utils.check_k8s_rollout('deployment', 'grafana', NAMESPACE)
    else:
